...
public class Main {
    ...
    // Знаходження мінімального елементу матриці
    ...
    // Знаходження максимального елементу матриці
    ...
    public static double calculateGeometricMean(int[][] matrix) {
        ...
        for (int[] row : matrix) {
            for (int value : row) {
                if (value == 0) {
                    return 0; // Середнє геометричне не визначено для нуля
                }
                product *= Math.abs(value); // Беремо абсолютне значення для уникнення негативних чисел
                count++;
            }
        }
